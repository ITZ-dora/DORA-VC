...
import config
from DORAMusic import Carbon, YouTube, app
...
from DORAMusic.utils.exceptions import AssistantErr
from DORAMusic.utils.inline.play import stream_markup, queue_markup, telegram_markup
from DORAMusic.utils.inline.playlist import close_markup
from DORAMusic.utils.pastebin import DORAbin
from DORAMusic.utils.stream.queue import put_queue, put_queue_index
from DORAMusic.utils.thumbnails import gen_thumb, gen_qthumb
from DORAMusic.utils.theme import check_theme
...
async def stream(
    ...
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await DORA.join_call(chat_id, original_chat_id, file_path, video=None)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await DORA.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
